...
valuationDate = datetime( 2015, 10, 1 );

% 1 EUR 
nominal = 1;
  
% Exercise date of the option = start date of swap
exerciseDate = valuationDate + years(2);
...
curveDates = daysadd(valuationDate, 360 * curveTimes, 1 );
...
settleDateNet = System.DateTime( year(valuationDate), month(valuationDate), day(valuationDate));
...
caplet =  PricingLibrary.FinancialInstruments.Caplet(exerciseDateNet, ...
...
yieldCurve = PricingLibrary.MarketData.YieldCurve(settleDateNet, curveDatesNet, yieldsNet);
...
seed = 41;
liborMarketModel = PricingLibrary.MarketModels.LiborMarketModel(yieldCurve, ...
                                                                beta, ...
                                                                volatiltyParametersNet, ... 
                                                                caplet.TenorStructure, ...
                                                                seed);
...
for i = 1 : numberOfMonteCarloPaths
    ...
    paths = liborMarketModel.SimulateForwardRatePathes(caplet.ExpiryDate);
    
    for j = 1 : length(strike) 
        caplet.Strike = strike(j);
        price(j) = price(j) + caplet.DiscountedPayoff(paths);
    end
end
